...
const PrayerJournal = () => {
  ...
  const filteredPrayers = savedPrayers.filter(prayer => {
    const matchesSearch = prayer.title.toLowerCase().includes(searchTerm.toLowerCase()) || prayer.preview.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  return <div className="min-h-screen bg-sacred-gradient">
      ...
      <div className="p-4 space-y-6 max-w-7xl mx-auto pb-24">
        ...
        <div className="flex items-center gap-4">
          <Input value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="Search your prayers..." className="border-sacred-golden-tan/30 focus:border-sacred-golden-tan bg-white flex-1 placeholder:text-gray-400" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredPrayers.length > 0 ? filteredPrayers.map(prayer => <Card key={prayer.id} className="bg-white/90 hover:bg-white/95 cursor-pointer transition-all border-sacred-golden-tan/20 hover:border-sacred-golden-tan h-full flex flex-col" onClick={() => navigate("/prayer-display", {
          state: {
            prayerId: prayer.id
          }
        })}>
                <CardContent className="p-4 flex flex-col justify-between h-full">
                  ...
                  <div className="flex justify-between items-center mt-4 pt-3 border-t border-sacred-golden-tan/20">
                    ...
                    <Button variant="ghost" size="sm" className="text-white border-none" style={{
                backgroundColor: '#d2b48c'
              }} onMouseEnter={e => e.currentTarget.style.backgroundColor = '#c19a6b'} onMouseLeave={e => e.currentTarget.style.backgroundColor = '#d2b48c'} onClick={e => {
                e.stopPropagation();
              ...
              <Card className="bg-white/90">
                <CardContent className="p-8 text-center">
                  ...
                  <Button onClick={() => navigate("/prayer-input")} className="bg-prayer-gradient hover:opacity-90 text-white">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-sm border-t border-sacred-golden-tan/20 p-4">
        <div className="flex justify-center items-center max-w-7xl mx-auto">
          <div className="flex justify-between items-center w-full max-w-md">
            <Button variant="ghost" onClick={() => navigate("/home")} className="flex flex-col items-center gap-1 text-sacred-sage-green hover:bg-sacred-cream/50 hover:text-sacred-sage-green/80">
